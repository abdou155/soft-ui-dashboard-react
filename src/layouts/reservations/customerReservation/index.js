...
import { listReservationsByUser } from "services/customer.service";
...
function CustomerReservation() {
 let navigate = useNavigate ();
  ...
  const fetchReservations = async () => {
    let response = await listReservationsByUser(id);
    if (response.data) {
      ...
      <SoftBox py={3}>
        <Card>
          <SoftBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
            ...
            <SoftButton onClick={() => { navigate("/customers") }} color="primary" size="small">
              Back
            </SoftButton>
          </SoftBox>
